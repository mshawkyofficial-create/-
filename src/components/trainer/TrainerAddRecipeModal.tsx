...
interface TrainerAddRecipeModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export const TrainerAddRecipeModal: React.FC<TrainerAddRecipeModalProps> = ({
  isOpen,
  onClose,
}) => {
  const { addCustomRecipe, language, t } = useApp();
  ...
  // Tags & Notes
  const [tagsInput, setTagsInput] = useState<string>('High Protein, Healthy');
  const [notes, setNotes] = useState<string>('');

  // Ingredients list
  const [ingredientsList, setIngredientsList] = useState<{ name: string; amount: string }[]>([
    { name: isRtl ? 'صدور دجاج منزوعة الجلد' : 'Skinless chicken breast', amount: '200g' },
    { name: isRtl ? 'أرز بسمتي مطبوخ' : 'Steamed Basmati Rice', amount: '150g' },
    { name: isRtl ? 'زيت زيتون بكر' : 'Extra virgin olive oil', amount: '10g' },
  ]);
  ...
  const [instructionsListEn, setInstructionsListEn] = useState<string[]>([
    ...
    'Cook in a non-stick pan until thoroughly done.',
    'Plate with carb base and enjoy warm.',
  ]);
  const [instructionsListAr, setInstructionsListAr] = useState<string[]>([
    'تتبيل البروتين بالملح والبهارات المفضلة.',
    'الطهي في مقلاة غير لاصقة حتى النضج التام.',
    'التقديم مع مصدر الكربوهيدرات وتناولها دافئة.',
  ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const trimmedEn = titleEn.trim() || titleAr.trim();
    const trimmedAr = titleAr.trim() || trimmedEn;
    const finalImage = imageUrl.trim() || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800&auto=format&fit=crop&q=80';
    ...
    const newRecipe: Recipe = {
      id: 'rec_custom_' + Date.now() + '_' + Math.random().toString(36).substring(2, 6),
      name: {
        en: trimmedEn,
        ar: trimmedAr,
      },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-xs animate-fade-in text-start">
      <div className="bg-white rounded-3xl p-5 sm:p-7 w-full max-w-2xl shadow-2xl border border-[#eceef0] max-h-[90vh] overflow-y-auto flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-black text-[#191c1e]">
                {isRtl ? 'إضافة وصفة جديدة لقاعدة البيانات' : 'Add Recipe to Database'}
              </h3>
              <p className="text-xs text-[#565e74]">
                {isRtl
                  ? 'إنشاء وصفة جديدة متكاملة مع الصورة، الفيديو، الماكروز والتحكم بنشرها.'
                  : 'Create a new recipe with image, video demo, macros, and visibility control.'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          {/* ======================================================== */}
          {/* 1. RECIPE VISIBILITY CONTROL                             */}
          {/* ======================================================== */}
          <div className="p-3 rounded-2xl bg-[#f7f9fb] border border-[#e0e3e5] flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-xs font-black text-[#191c1e] block">
                  {isRtl ? 'حالة النشر للمتدربين' : 'Publish Status'}
                </span>
                <span className="text-[10px] text-[#565e74]">
                  {published
                    ? isRtl ? 'منشورة وظاهرة في مكتبة المتدربين' : 'Published & Visible in Client Recipe Library'
                    : isRtl ? 'مسودة مخفية (تظهر للمدرب فقط)' : 'Hidden Draft (Trainer Only)'}
                </span>
              </div>
            </div>
            ...
            >
              {published ? (isRtl ? 'منشورة / Published' : 'Published') : (isRtl ? 'مخفية / Hidden' : 'Hidden')}
            </button>
          </div>

          {/* ======================================================== */}
          {/* 2. RECIPE IMAGE SELECTOR & LIVE PREVIEW                  */}
          {/* ======================================================== */}
          <div className="p-4 sm:p-5 rounded-2xl bg-[#f8faf2] border border-[#506600]/25 flex flex-col gap-3.5">
            ...
            <div className="relative w-full h-44 sm:h-48 rounded-2xl overflow-hidden bg-slate-900 border-2 border-dashed border-[#506600]/40 group">
              {imageUrl ? (
                <>
                  <img
                    src={imageUrl}
            ...
            {/* Image Selector Tabs: Presets / Upload / URL */}
          ...
          <div className="p-4 sm:p-5 rounded-2xl bg-[#f0f7fc] border border-[#0284c7]/30 flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <label className="text-sm font-black text-[#191c1e]">
                  {isRtl ? 'رابط فيديو الوصفة (اختياري - YouTube / Vimeo / Direct)' : 'Recipe Video URL (Optional - YouTube / Vimeo / Direct)'}
                </label>
              </div>
            </div>
            ...
            <p className="text-xs text-[#565e74]">
              {isRtl
                ? 'يمكنك ترك الرابط فارغاً وإضافته لاحقاً. ستظهر أيقونة الفيديو دوماً، وتخبر المتدرب بأنه غير متاح حالياً إذا كان فارغاً.'
                : 'Leave empty or add a link anytime. Video badge will show on every recipe card with a clear notice if unpopulated.'}
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              ...
              <button
                ...
                className="h-10 px-3.5 rounded-xl bg-[#0284c7] hover:bg-[#0369a1] text-white text-xs font-bold flex items-center justify-center gap-1.5 transition-colors shadow-xs shrink-0"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Garlic Herb Chicken Bowl"
                required
            ...
            <div>
              ...
              <input
                ...
                placeholder="مثال: وعاء الدجاج بالأعشاب والثوم"
                className="w-full h-10 px-3 rounded-xl bg-white border border-[#e0e3e5] text-xs font-semibold text-[#191c1e] focus:border-[#506600] outline-none"
              />
            </div>
          </div>
          ...
          {/* Form Actions */}
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-[#eceef0]">
            <button
              type="button"
              onClick={onClose}
            ...
            >
              {saveSuccess ? (
                <>
                  ...
                  <span>{isRtl ? 'تمت إضافة الوصفة بنجاح!' : 'Recipe Added!'}</span>
                </>
              ) : (
                <>
                  <Plus className="w-4 h-4" />
                  <span>{isRtl ? 'إضافة الوصفة إلى قاعدة البيانات' : 'Add Recipe to Database'}</span>
                </>
              )}
            </button>
          </div>
